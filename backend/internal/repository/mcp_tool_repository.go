...
func (r *MCPToolRepository) Create(ctx context.Context, req models.CreateMCPToolRequest) (*models.MCPTool, error) {
	...
	query := `
		...
		RETURNING id, server_id, name, description, server_url, transport_type, input_schema, created_at
	`
	var t models.MCPTool
	err := r.pool.QueryRow(ctx, query, toolID, serverID, req.Name, req.Description, req.ServerURL, transport, req.InputSchema, now).Scan(
...
func (r *MCPToolRepository) GetByID(ctx context.Context, id uuid.UUID) (*models.MCPTool, error) {
	query := `SELECT id, server_id, name, description, server_url, transport_type, input_schema, created_at FROM mcp_tools WHERE id = $1`
	var t models.MCPTool
	err := r.pool.QueryRow(ctx, query, id).Scan(&t.ID, &t.ServerID, &t.Name, &t.Description, &t.ServerURL, &t.TransportType, &t.InputSchema, &t.CreatedAt)
	if err != nil {
...
func (r *MCPToolRepository) List(ctx context.Context) ([]models.MCPTool, error) {
	query := `SELECT id, server_id, name, description, server_url, transport_type, input_schema, created_at FROM mcp_tools ORDER BY created_at DESC`
	...
	for rows.Next() {
		var t models.MCPTool
		if err := rows.Scan(&t.ID, &t.ServerID, &t.Name, &t.Description, &t.ServerURL, &t.TransportType, &t.InputSchema, &t.CreatedAt); err == nil {
